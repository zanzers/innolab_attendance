...
class _AttendanceScreenState extends State<AttendanceScreen> {
  // Use 'late' or initialize directly
  final controller = AttendanceProvider();
  ...
  void initState() {
    ...
    // 2. Formatting Contact Number logic
    ...
    // 3. Listen to the provider to trigger UI updates
    controller.addListener(() {
      if (mounted) {
        setState(() {
          // Sync local bools with provider if necessary
          if (controller.selectedSex == 'M') { _isMale = true; _isFemale = false; }
          else if (controller.selectedSex == 'F') { _isFemale = true; _isMale = false; }
          
          // If the provider was forceReset(), reset local UI bools
          if (!controller.isUserCheckedIn && controller.contact.text.isEmpty) {
            _isMale = false;
            _isFemale = false;
